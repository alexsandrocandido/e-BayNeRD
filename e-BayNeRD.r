...
FUNCTION_how.to.cite.the.program.bibtex <- function() {
  # Shows how to cite this program in BibTex
  cat("\n\n
...
FUNCTION_eBayNeRD.R.menu.names <- function() {
  ...
  menu.type <- c("A0","A1","B0","B0",
                 "A0","A0","A1","B0","B0",
                 "A0","A0","A0","A0","A0",
                 "A1","B0","B0","B0","B0","B0","B0","B0",
                 "A0")
  menu.disable.until.step <- c(0,0,0,0,0,1,2,2,2,3,4,5,5,6,0,7,7,7,7,7,7,7,0)
  menu.name <- c("About",
  ...
  menu.action <- c("FUNCTION_write.the.heading.of.the.program()",
                   "",
                     ...
                     "FUNCTION_how.to.cite.the.program.bibtex()",
...
FUNCTION_read.context.variables <- function(subfolder = T) {
  ...
  flush.console()
  cat("\n\n
      Writing a copy of each context variable raster\n",
      "in './e-BayNeRD Outcomes' folder\n\n")
...
FUNCTION_graphical.model <- function() {    
  ...
  if (Sys.info()[[1]] == 'Windows') {
    ...
    cat('\n\n
        Done! In the next step you should use 
        the function FUNCTION_prob.table().\n\n')
  }
...
FUNCTION_probability.table <- function(){
  ...
  if(Sys.info()[[1]]=='Windows')
  {
    ...
    cat('\n\n
        Done! In the next step you should use 
        the function FUNCTION_probabilities.bands().\n\n
        You can also see the influence of each context variable
        in the calculated probability for the TARGET one by using
        the function FUNCTION_running.influence.KLscore().\n\n')
  }
  invisible(gc())
  invisible(rm(list=ls()))
}
...
FUNCTION_probabilities.bands <- function() {
...
FUNCTION_read.target.variable.for.testing <- function() {
  ...
  ok <- F
  while(ok==F)  {
    is.it.ok <- readline("Do you want to use the same TARGET image used as reference for training? (y-Yes ; n-No): ")
  ...
  if(Sys.info()[[1]]=='Windows')
  {
    ...
    FUNCTION_eBayNeRD.R.menu.activation(menu.elements=eBayNeRDinfo$menu,exist.eBayNeRDinfo=T)
  } else {cat('\n\nDone! In the next step you may use the function FUNCTION_assessment().\n\n')}
...
FUNCTION_find.bestTPV<-function(n.slices = 101, criterion.number = 1) {
  ...
  cat("\n\n")
  cat(paste("Based on the criterion: '",criterion,"'\nthe selected best-TPV was:\n",sep=""))
  print(result2[bestTPV,])
  cat("\n\nIt were generated two figures related to this procedure.\n\n")
  cat("\nSummary about the ROC curve:\n\n")
  print(ROC.curve)
  cat("\n\n\n")
...
FUNCTION_working.folder <- function() {
  ...
  cat("\n\n
      ...
      working.folder, "\n\n")
  resp <- readline("Do you want to change it? (y-Yes ; n-No): ")
  if ((resp == "y") | (resp == "Y")) {
    cat("\n\n")
...
FUNCTION_before.beginning <- function() {    
  ...
  if (sum(dir() == "eBayNeRDinfo.RData") == 0) {
    ...
    cat("\n
        It was verified that a previous Bayesian Network process 
        had been started before.\n
        Do you want to:\n
        (1) Start a new process .
            BE CAREFUL!!! 
            All previous work done by the e-BayNeRD algorithm 
            in this folder will be lost!\n
...
if (Sys.info()[[1]] == "Windows") {
  ...
  invisible(FUNCTION_write.the.heading.of.the.program())
  cat("\n\n
